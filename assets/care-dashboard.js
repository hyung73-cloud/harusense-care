(function () {
  ...
  function renderList(items) {
    ...
    list.innerHTML = items.map((submission) => {
      const visitLabel = window.careApp.careVisitLabel(submission.visitNumber, submission.templateId);
      const medLine = submission.templateId === "initial"
        ? `키 ${window.careApp.careEscapeHtml(submission.heightCm || "__")}cm · 현재 ${window.careApp.careEscapeHtml(submission.currentWeightKg)}kg`
        : `${window.careApp.careEscapeHtml(submission.currentMedication || "약제 미입력")} ${window.careApp.careEscapeHtml(submission.currentDose)} · 현재 ${window.careApp.careEscapeHtml(submission.currentWeightKg)}kg`;

      return `
      <button class="care-submission-item" type="button" data-id="${submission.id}" aria-selected="${submission.id === selectedId ? "true" : "false"}">
        <strong>${window.careApp.careEscapeHtml(submission.name)} · ${visitLabel} · ${window.careApp.careFormatTime(submission.submittedAt)}</strong>
        <span>${medLine}</span>
  ...
  function renderDetail(submission, summary, history) {
    ...
    detail.innerHTML = `
      <article class="care-detail">
        <div>
          <p class="care-eyebrow">${window.careApp.careEscapeHtml(visitLabel)} · ${window.careApp.careFormatTime(submission.submittedAt)} 제출</p>
        ...
        <section class="care-kv">
          ...
          ${submission.templateId === "initial"
            ? `<div><span>키</span>${window.careApp.careEscapeHtml(submission.heightCm || "__")}cm</div>`
            : ""}
