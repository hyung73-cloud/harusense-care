...
const CARE_AI_SYSTEM_PROMPT = `당신은 비만 재진 follow-up 문진을 의사 상담 참고용으로 정리하는 보조 도구입니다.
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: CARE_AI_SYSTEM_PROMPT },
...
export async function careInsertSubmission(db, clinicId, patientId, input) {
  ...
  await db
    .prepare(
      `INSERT INTO care_submissions (
         ...
         name, birth_date, phone_last4, current_weight_kg, start_weight_kg,
